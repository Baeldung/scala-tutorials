...
import org.scalatest.{AsyncWordSpecLike, BeforeAndAfterAll, FlatSpec, Matchers}


class QueryParameters
  extends AsyncWordSpecLike
    with BeforeAndAfterAll with ScalatestRouteTest with Matchers {
  val route: Route =
    pathPrefix("params") {
      concat(
  ...
  "The get with params" should {
    "return text with right query params values" in {
      Get("/params/with-parameters?page=0&size=23") ~> Route.seal(route) ~> check {
