...
trait ForLoop {
  ...
  def forComprehensionWithOptionals(
    someIntValue: Option[Int],
    someStringValue: Option[String]
  ): Option[String] = {
    for {
  ...
  def mapOptionals(
    ...
    someIntValue.flatMap(intValue =>
      someStringValue.map(stringValue => s"""$intValue is $stringValue""")
    )
  }
}
