...
class FutureRecoveryUnitTest extends AsyncFlatSpec {
  ...
  private val forecastService =
    new FutureRecovery.WeatherForecastService(httpClient)
  ...
  it should "return the previous forecast in case of error" in {
    forecastService.forecast("2020-10-19") andThen { case _ =>
      forecastService.forecast("2020-10-20")
    } map { forecast =>
  ...
  "A forecast with fallback" should "return the current forecast" in {
    forecastService.forecast("2020-10-18", "http://weather.com") map {
      forecast =>
        assert(forecast == Sunny)
    }
  }
  ...
  it should "return the previous forecast in case of error" in {
    forecastService.forecastUsingMapAndRecover("2020-10-19") andThen { case _ =>
      forecastService.forecastUsingMapAndRecover("2020-10-20")
    } map { forecast =>
      assert(forecast == Windy)
    }
  }
