...
class PatternMatching {
  ...
  def caseClassesPatternMatching(animal: Animal): String = {
    animal match {
      case Mammal(name, fromSea) =>
        s"I'm a $name, a kind of mammal. Am I from the sea? $fromSea"
  ...
  def tuplesPatternMatching(tuple: Any): String = {
    tuple match {
      ...
      case (first, second, third) =>
        s"I'm a tuple with three elements: $first & $second & $third"
  ...
  def regexPatterns(toMatch: String): String = {
    ...
    toMatch match {
      ...
      case _ =>
        s"I contain other characters than alphanumerics. My value $toMatch"
    }
  }
