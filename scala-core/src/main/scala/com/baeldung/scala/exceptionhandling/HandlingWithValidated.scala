...
object HandlingWithValidated {
  ...
  val passwordPattern = "(?=^.{10,}$)((?=.*\\d)|(?=.*\\W+))(?![.\\n])(?=.*[A-Z])(?=.*[a-z]).*$".r

  def validateLogin (login: String): InvalidOr[String] =
    Option(login).flatMap(loginPattern.findFirstIn(_)).map(_.validNel).getOrElse(IllegalLogin(login).invalidNel)

  def validatePassword(password: String): InvalidOr[String] =
    Option(password).flatMap(passwordPattern.findFirstIn(_)).map(_.validNel).getOrElse(IllegalPassword(password).invalidNel)
