...
object HandlingWithMonadError {
  ...
  def monadErrorGetResource[F[_], E] (resourceId: String, session: Session)(implicit me: MonadError[F, E], adoptError: LegacyErrors => E): F[Resource] = {
  ...
  def getResourceValue[F[_], E] (user: User, resoureceId: String)(implicit me: MonadError[F, E], adoptError: LegacyErrors => E): F[String] = {
    for {
