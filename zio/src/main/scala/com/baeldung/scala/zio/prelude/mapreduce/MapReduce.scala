...
object MapReduce {
  ...
  def mapReduce[A, B](as: List[A])(f: A => B)(implicit
    identity: Identity[B]
  ): B =
    as.map(f).toNonEmptyList.map(_.reduce1).getOrElse(identity.identity)

  def mapReduce2[F[+_]: ForEach, A, B: Identity](as: F[A])(f: A => B): B =
    as.foldMap(f)
  ...
  def wordCountVeryAbstract(as: List[String]): Int =
    mapReduce2(as)(wordsPerLine)
