...
object SubListOfList {
  ...
  def usingFoldLeftAllowGaps(bigList: List[String], littleList: List[String]): Boolean = {
    bigList.foldLeft(littleList)((lList, string) => {
      val isStringInList = lList.contains(string)
      if (isStringInList) lList.tail else lList
    }).isEmpty
  }

  def usingFoldLeftNoGaps(bigList: List[String], littleList: List[String]): Boolean = {
    bigList.foldLeft(littleList)((lList, string) => {
  ...
  def usingContainsSlice(bigList: List[String], littleList: List[String]): Boolean = {
