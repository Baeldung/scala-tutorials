...
object FutureFailuresList {
  ...
  private def usingRecover(
    futures: List[Future[String]]
  ): Seq[Future[Try[String]]] = {
    futures.map(f => {
      f.map(Success(_)).recover { case x => Failure(x) }
    })
  }

  def getSuccessfulUsingRecover(
    futures: List[Future[String]]
  ): Future[Seq[String]] = {
    val futuresSeq = Future.sequence(usingRecover(futures))
    futuresSeq.map(f =>
      f.filter { _.isSuccess }.collect { case Success(str) => str }
    )
  }

  def getFailuresUsingRecover(
    futures: List[Future[String]]
  ): Future[Seq[Throwable]] = {
    val futuresSeq = Future.sequence(usingRecover(futures))
    futuresSeq.map(f =>
      f.filter { _.isFailure }.collect { case Failure(ex) => ex }
    )
  }
