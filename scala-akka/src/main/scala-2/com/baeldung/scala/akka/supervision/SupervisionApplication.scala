...
import com.baeldung.scala.akka.supervision.SupervisionApplication.Filesystem.{
  FsFind,
  FsFound
}
...
object SupervisionApplication {
  ...
  object Main {
    ...
    def apply(): Behavior[Start] = {
      Behaviors
        .receive[Start] { (context, message) =>
          val webServer = context.spawn(WebServer(), message.id)
          context.watch(webServer)
          message.replyTo ! Created(webServer)
          Behaviors.same
        }
        .receiveSignal { case (ctx, ChildFailed(ref, cause)) =>
          ctx.log.error(
            s"Child actor ${ref.path} failed with error ${cause.getMessage}"
          )
          Behaviors.same
        }
    }
  ...
  trait Resource
  case class File(id: String, content: Array[Byte], mimeType: String)
    extends Resource
  ...
  object WebServer {
    ...
    case class AdaptedHitResponse(
      path: String,
      resource: Resource,
      replyTo: ActorRef[Response]
    ) extends Request

    case class AdaptedMissResponse(path: String, replyTo: ActorRef[Response])
      extends Request

    case class AdaptedErrorResponse(
      path: String,
      replyTo: ActorRef[Response],
      error: String
    ) extends Request
    ...
    private def findInCache(
      ...
      context.ask(
        cache,
        (ref: ActorRef[Cache.Response]) => Cache.Find(path, ref)
      ) {
        case Success(Cache.Hit(resource)) =>
          AdaptedHitResponse(path, resource, replyTo)
      ...
      Behaviors
        ...
        .onFailure[Exception](
          SupervisorStrategy.restart
            .withLimit(maxNrOfRetries = 10, withinTimeRange = 5.minutes)
        )
    }

    private def search: Behavior[FsFind] =
      Behaviors
        .receive[FsFind] { (context, message) =>
          context.log.info(s"Received a request for path ${message.path}")
          val response =
            if (message.path.contains("valid"))
              FsFound(
                File("id", "{'result': 'ok'}".getBytes(), "application/json")
              )
            else
              FsMiss
  ...
  object Cache {
    ...
    case class AdaptedFsFound(
      path: String,
      resource: Resource,
      replyTo: ActorRef[Response]
    ) extends Request

    case class AdaptedFsMiss(path: String, replyTo: ActorRef[Response])
      extends Request
        ...
        message match {
          case Find(path, replyTo) =>
            ...
            maybeAnHit match {
              ...
              case None =>
                askFilesystemForResource(filesystem, ctx, path, replyTo)
            }
            Behaviors.same
    ...
    private def askFilesystemForResource(
      filesystem: ActorRef[FsFind],
      ...
      ctx.ask(
        filesystem,
        (ref: ActorRef[Filesystem.FilesystemResponse]) => FsFind(path, ref)
      ) {
        case Success(FsFound(resource)) =>
          AdaptedFsFound(path, resource, replyTo)
