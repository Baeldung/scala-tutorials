...
object KittensServer extends App with BaseAkkaServer {
        ...
        if (kitten.id <= 0) {
          ...
          if (DB.kittens.exists(_.id == kitten.id)) {
            Future.successful(
              Left(
                StatusCode.BadRequest -> ErrorResponse(
                  s"kitten with id ${kitten.id} already exists"
                )
              )
            )
          } else {
  ...
  val putKittens = AkkaHttpServerInterpreter().toRoute(
    AnimalEndpoints.kittensPut
      .serverLogic(kitten => {
        val updatedKittenOpt = DB.kittens
          .find(_.id == kitten.id)
          .map(
            _.copy(
              name = kitten.name,
              gender = kitten.gender,
              ageInDays = kitten.ageInDays
            )
          )
        updatedKittenOpt
          .map(updatedKitten => {
            DB.kittens =
              DB.kittens.filterNot(_.id == kitten.id) :+ updatedKitten
            ...
            Future.successful(
              Left(
                StatusCode.NotFound -> ErrorResponse(
                  s"kitten with id ${kitten.id} was not found"
                )
              )
            )
          )
        ...
        deletedKittenOpt
          ...
          .getOrElse(
            Future.successful(
              Left(
                StatusCode.NotFound -> ErrorResponse(
                  s"kitten with id $kittenId was not found"
                )
              )
            )
          )
