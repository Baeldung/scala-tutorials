...
object GetClient extends App {
  ...
  val kittenRequest
    : Unit => Request[DecodeResult[Either[String, List[Kitten]]], Any] =
    SttpClientInterpreter()
      .toRequest(AnimalEndpoints.kittens, Some(uri"http://localhost:11223"))

  val backend = HttpClientSyncBackend()
  val kittensR: RequestT[Identity, String, Any] =
    kittenRequest().response(asStringAlways)
...
object ClientPost extends App {
  ...
  val kittenPostRequest: Kitten => Request[DecodeResult[
    Either[(StatusCode, ErrorResponse), (StatusCode, Kitten)]
  ], Any] =
    SttpClientInterpreter()
  ...
  val chaseR: RequestT[Identity, String, Any] =
    kittenPostRequest(Kitten(12L, "chase", "male", 14)).response(asStringAlways)
